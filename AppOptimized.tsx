...
import { generateSingleCharacter, generateCharacterWithNpcs } from './services/geminiServiceOptimized';
...
const App: React.FC = () => {
  ...
  // Optimized NPC generation that could be enhanced to generate in batch with character if needed
  ...
  const handleGenerateWithNpcs = useCallback(async (prompt: string) => {
    ...
    try {
      ...
      // Create NPCs
      const newNpcs: StoredCharacter[] = result.npcs.map(npc => ({
        id: crypto.randomUUID(),
        system: selectedSystem,
