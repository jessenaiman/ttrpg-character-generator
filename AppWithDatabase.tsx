...
import { GameSystem, DbStoredCharacter } from './database';
...
const App: React.FC = () => {
  ...
  const [characters, setCharacters] = useState<DbStoredCharacter[]>([]);
  const [selectedCharacter, setSelectedCharacter] = useState<DbStoredCharacter | null>(null);
  ...
  useEffect(() => {
    const loadCharacters = async () => {
      try {
        setIsLoading(true);
        const dbCharacters = await DatabaseService.getAllCharacters();
  ...
  // Update the database when characters change
  useEffect(() => {
    // Don't save to DB while initially loading
    if (characters.length > 0 && !isLoading) {
      // Use a timeout to avoid excessive writes on every change
      const saveTimeout = setTimeout(() => {
        DatabaseService.bulkUpsert(characters)
          .catch(err => console.error('Failed to save characters to database:', err));
  ...
  const handleGenerate = useCallback(async (prompt: string) => {
    ...
    try {
      ...
      const newCharacter: DbStoredCharacter = {
  ...
  const handleGenerateNpc = useCallback(async (parentCharacter: DbStoredCharacter) => {
    setIsLoading(true);
    setError(null);
    setSelectedCharacter(null);
    ...
    try {
      ...
      const newNpc: DbStoredCharacter = {
        id: crypto.randomUUID(),
  ...
  const handleDeleteCharacter = async (idToDelete: string) => {
    try {
      await DatabaseService.deleteCharacter(idToDelete);
  ...
  const handleSelectCharacter = (characterToView: DbStoredCharacter) => {
